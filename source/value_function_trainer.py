...
def train_network(model, train_data, valid_data, option):
    ...
    train_loader = torch.utils.data.DataLoader(train_data, batch_size=option.batch_size, shuffle=True, num_workers=2, collate_fn=collate_fn_pad)
    test_loader = torch.utils.data.DataLoader(valid_data, batch_size=option.batch_size, shuffle=True, num_workers=2, collate_fn=collate_fn_pad)
    ...
    for epoch in range(option.epochs):
        ...
        logging.info('Epoch: {:>2}, Train Loss: {:.4f}, Validate Loss: {:.4f}'.format(epoch, epoch_loss_train, epoch_loss_test))
...
def init(option):    
    ...
    # Train
    logging.info('Training value function model on {}'.format('full dataset' if option.full_train else '0.8, validating on 0.2'))
